...
from dictdiffer._compat import MutableMapping, MutableSequence
...
class DictDifferTests(unittest.TestCase):
    ...
    def test_tolerance(self):
        ...
        diffed = sorted(diff(first, second, tolerance=0.01))
        assert [
            ("change", "a", (10.0, 10.5)),
            ("change", "b", (1.0e-15, 2.5e-15)),
        ] == diffed

        diffed = sorted(diff(first, second, tolerance=0.01, absolute_tolerance=1e-12))
        assert [("change", "a", (10.0, 10.5))] == diffed

        diffed = sorted(diff(first, second, tolerance=0.01, absolute_tolerance=1e-18))
        assert [
            ("change", "a", (10.0, 10.5)),
            ("change", "b", (1.0e-15, 2.5e-15)),
        ] == diffed

        diffed = sorted(diff(first, second, tolerance=0.1, absolute_tolerance=1e-18))
        assert [("change", "b", (1.0e-15, 2.5e-15))] == diffed

        diffed = sorted(diff(first, second, tolerance=0.1, absolute_tolerance=1e-12))
        assert [] == diffed

        diffed = sorted(diff(first, second, tolerance=None, absolute_tolerance=None))
        assert [
            ("change", "a", (10.0, 10.5)),
            ("change", "b", (1.0e-15, 2.5e-15)),
        ] == diffed
    ...
    def test_unicode_keys(self):
        ...
        assert ("remove", "", [("привет", 1)]) in diffed

        diffed = list(diff(first, second, ignore=["hello"]))
        assert ("remove", "", [("привет", 1)]) == diffed[0]

        diffed = list(diff(first, second, ignore=["привет"]))
    ...
    def test_ignore_with_unicode_sub_keys(self):
        ...
        assert len(list(diff(first, second, ignore=["a.aא.aa"]))) == 0
        assert len(list(diff(first, second, ignore=[["a", "aא", "aa"]]))) == 0
    ...
    def test_ignore_with_ignorecase(self):
        ...
        assert list(
            diff({"a": 1, "b": 2}, {"A": 3, "b": 4}, ignore=IgnoreCase("a"))
        ) == [("change", "b", (2, 4))]
...
@pytest.mark.parametrize(
    ...
    [
        ("nifi.zookeeper.session.timeout", True, 1),
        ("nifi.zookeeper.session.timeout", False, 0),
        (("nifi.zookeeper.session.timeout",), True, 0),
        (("nifi.zookeeper.session.timeout",), False, 0),
    ],
)
def test_ignore_dotted_ignore_key(ignore, dot_notation, diff_size):
    key_to_ignore = "nifi.zookeeper.session.timeout"
    config_dict = OrderedDict(
        [("address", "devops011-slv-01.gvs.ggn"), (key_to_ignore, "3 secs")]
    )

    ref_dict = OrderedDict(
        [("address", "devops011-slv-01.gvs.ggn"), (key_to_ignore, "4 secs")]
    )

    assert diff_size == len(
        list(diff(config_dict, ref_dict, dot_notation=dot_notation, ignore=[ignore]))
    )
